    ...
    func checkSubscriptionAtLaunch() async {
        ...
        if let expiry = expirationDate {
            if expiry < Date() {
                ...
                isPro = false
                UserdefaultManager.shared.savePro(false)
            } else {
                isPro = true
...
//            UserDefaultClass.sharedInstanse.setUpValue(key: userdefaultKey.isPro, Value: )
//            UserDefaults.standard.set(foundActive, forKey: SessionKeys.isPro)
            isPro = foundActive
            UserdefaultManager.shared.savePro(foundActive)
        } catch {
